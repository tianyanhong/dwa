...
void DWAPlanner::load_params(void)
{
  ...
  local_nh_.param<double>("MAX_IN_PLACE_YAWRATE", max_in_place_yawrate_, 0.6); //机器人原地旋转允许的最大角速度
  local_nh_.param<double>("MAX_VELOCITY", max_velocity_, 1.0);
  local_nh_.param<double>("MAX_YAWRATE", max_yawrate_, 1.0);
  local_nh_.param<double>("MIN_IN_PLACE_YAWRATE", min_in_place_yawrate_, 0.0);
  ...
  local_nh_.param<double>("PREDICT_TIME", predict_time_, 10.0); //预测的时间长度
  // - R -
  ...
  local_nh_.param<double>("SIM_PERIOD", sim_period_, 10);//轨迹预测的总时间，dwa会模拟机器人接下来n秒的运动轨迹
  local_nh_.param<int>("SIM_TIME_SAMPLES", sim_time_samples_, 2); //轨迹的采样点数 /dwa_planner/selected_trajectory由100改为10  频率由1变成了10
  ...
  // - T -
  local_nh_.param<double>("TARGET_VELOCITY", target_velocity_, 0.55); //2.0
  local_nh_.param<double>("TO_GOAL_COST_GAIN", to_goal_cost_gain_, 0.8);
  local_nh_.param<double>("TURN_DIRECTION_THRESHOLD", turn_direction_th_, 0.1); //是否允许旋转的角度阈值
  // - U -
